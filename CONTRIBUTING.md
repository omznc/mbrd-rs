...
### And GStreamer, for sound and video

`crates/app/src/pipeline.rs` is the only part of mbrd that talks to anything
outside the process, and it links against the desktop's own media stack rather
than carrying decoders of its own:

```
sudo dnf install gstreamer1-devel gstreamer1-plugins-base-devel   # Fedora
sudo apt install libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev  # Debian
```

You want the plugin sets too — `gstreamer1-plugins-good` and
`gstreamer1-plugin-libav` on Fedora, `gstreamer1.0-plugins-good` and
`gstreamer1.0-libav` on Debian — or the app builds and runs but every clip
answers that it has no decoder. A desktop that plays video in its file manager
already has all of them.

**This install step is Linux-only, and so is `pipeline.rs`.** `main.rs` picks
the file by target: `pipeline_mac.rs` on macOS (AVFoundation), `pipeline_win.rs`
on Windows (the Media Foundation Media Engine), `pipeline_off.rs` on anything
else. All three play; only Linux asks you to install its decoder first, because
only on Linux is the media stack a package rather than part of the OS.

Four files behind one door, and the door is the thing to keep an eye on. Every
backend is the same handful of methods — `open`, `play`, `pause`, `seek`,
`poll`, `picture`, `forget` — and `board_view.rs` knows only those. If you find
yourself adding a method for one platform, that is the signal to look again:
the last three times, the honest version was a method all three could answer.

Two of them cannot be compiled here. `cargo check --target x86_64-apple-darwin`
and `--target x86_64-pc-windows-msvc` will not get through GPUI, so a change
that touches them is a change CI type-checks and you do not. `spill.rs` is
where the testable part of all three went, on purpose.
...
| `core/paper.rs` | sheet sizes, the scale bar's round number, and calibration both ways |
| `core/tags.rs` | what a tag may be, in any language somebody has — a block-list, not an allow-list |
| `core/tour.rs` | the saved route; **the route is board data, where you got to is not** |
| `core/inventory.rs` | what a board is made of and weighs, **without decoding anything** |
| `core/shrink.rs` | which pictures are worth re-encoding, and what a run may not touch |
| `core/formats.rs` | 1,367 file names, generated by `scripts/formats.mjs` — **do not hand-edit** |
...
| `app/stock.rs` | the inventory page, and the one button that acts on it |
| `app/shrink.rs` | the re-encode itself: JPEG or PNG, decided by looking at the pixels |
